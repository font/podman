...
// cosignSignature is the payload format for sigstore signing an image using
...
var _ json.Marshaler = (*cosignSignature)(nil)

// MarshalJSON implements the json.Marshaler interface.
func (s cosignSignature) MarshalJSON() ([]byte, error) {
...
var _ json.Unmarshaler = (*cosignSignature)(nil)

// UnmarshalJSON implements the json.Unmarshaler interface
func (s *cosignSignature) UnmarshalJSON(data []byte) error {
	err := s.strictUnmarshalJSON(data)
	if err != nil {
		if formatErr, ok := err.(jsonFormatError); ok {
			err = InvalidSignatureError{msg: formatErr.Error()}
...
// strictUnmarshalJSON is UnmarshalJSON, except that it may return the internal jsonFormatError error type.
// Splitting it into a separate function allows us to do the jsonFormatError → InvalidSignatureError in a single place, the caller.
func (s *cosignSignature) strictUnmarshalJSON(data []byte) error {
	var critical, optional json.RawMessage
	if err := paranoidUnmarshalJSONObjectExactFields(data, map[string]interface{}{
		"critical": &critical,
		"optional": &optional,
	...
	return paranoidUnmarshalJSONObjectExactFields(identity, map[string]interface{}{
		"docker-reference": &s.DockerReference,
	})
}
